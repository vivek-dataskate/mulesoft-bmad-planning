...
**Client-logo sizing/quality is owned by CSS only, not by selection logic.** Vera scrapes a logo URL (og:image → apple-touch-icon → icon) → it's downloaded to `projects/{slug}/intake/logo-{slug}.png` + `firebase/public/logos/{slug}.png` → templates read the `firebase/public/logos/` copy and inject `<img class="client-logo-img">`, sized by `.client-logo-img { height:32px; max-width:140px; object-fit:contain }`. Layout never breaks, but image *selection* is unvalidated (og:image is often a wide banner, not a logo) and there is no human review gate before the client-facing corporate brief ships. Known, accepted gap as of 2026-05-20 — see the logo-flow note in `docs/HTML_PIPELINE_MIGRATION.md` risk surface before changing it.
...
**FROZEN TEMPLATE — `commons/templates/intake-template.html`:** This file is frozen as of 2026-05-20. The approved baseline is the 2-column layout: main column with sticky category tabs (Business / Technical / Production / Future Flows) + collapsible section tiles, plus a sticky right rail with Progress / Subsections / Needs Attention / Links. Header is 2-column (title left, meta right with hyperlinked Source). Biz-context is teaser-by-default. Sticky bar has Expand-all / Collapse-all tools on the left. Submit writes to Firestore `intake_responses/{client}.versions[]` via `arrayUnion` (versioning + soft-lock banner — no hard lock). Print/Save PDF force-opens all `<details>` via JS. Architect Guide MUST NOT appear in rail-links. Pitch Kit MUST NOT appear in client portal. Do NOT modify this file without explicit user approval — if a change is needed, state what will change and wait for confirmation before touching the file.
