...
- `standards/playbooks/{system}/` — the authoritative source for system-specific field knowledge. Each playbook contains PLAYBOOK.md (quirks, auth, supported objects, maturity log) + DataWeave mapping files (canonical ↔ system) + reusable Mule XML sub-flows.
...
- `standards/usecases/{vertical}.json` — use case library, one file per industry vertical (b2b-saas, construction, healthcare, etc.). Each entry is tagged `source: "web"` (researched from public case studies) or `source: "DS"` (from a real DataSkate engagement). DS entries include `clientContext: "projects/{slug}/"` and `clients: ["{slug}"]`. Scout reads this before web research and writes new discoveries back after each session. Schema: id, vertical, sizeSegment (smb/midmarket/enterprise), systems[], useCase, whatWasBuilt, outcome, aiLayer, source, sourceCompany, sourceUrl, addedDate, clients[].
...
- `standards/field-schemas/` — DO NOT CREATE files here. This directory should not exist. System field knowledge belongs in `standards/playbooks/{system}/PLAYBOOK.md`.
