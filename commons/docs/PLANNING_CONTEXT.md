...
BMAD ARCHITECT AGENT
  Reads prd.md + commons/docs/DESIGN_STANDARDS.md
...
It is **not** the system definition — that is `commons/docs/DESIGN_STANDARDS.md`.
...
- **ComputerEase (Deltek) is NOT directly internet-accessible — CE Live Service relay required:** ComputerEase APIs route through a vendor-managed Deltek URL that proxies to a 'CE Live Service' Windows service installed on the customer's VM. Customer-side firewall rules on standard HTTPS ports (443/8081) are irrelevant — the service typically listens on TCP/445 internally. Day-1 actions: (a) open Deltek support ticket to provision CE Live Service, (b) get the Deltek-managed integration URL, (c) treat customer VM IP only as outbound-egress concern. Vendor SLA is 4 hours minimum (product is classified legacy). Sandbox/practice tenant does NOT expose APIs — testing must be GET-only in production until vendor provisions a dev tenant. (Source: peerless transcript, 2026-04-17 — Raghuram Potluri research finding.)
...
- **IDP + deterministic validation — two-stage pipeline pattern (portable to all Elevate-style signable-document flows):** When MuleSoft IDP extracts fields from compliance documents, NEVER embed business-rule validation inside the AI extraction prompt. Use a two-stage pipeline: Stage 1 = IDP extraction only (raw fields + confidence scores → structured JSON). Stage 2 = DataWeave/Java deterministic validation (explicit rule per business requirement, unit-testable without IDP). State-specific rules (e.g., WI 99% deposit threshold vs. IL/IN/OH 100%) are constants in a DataWeave config file — each new state added by appending one line. This pattern is confirmed portable: any Elevate sibling brand (Imperial Fence, Akoya Pools, etc.) deploying the same document validation flow inherits Stage 2 with only the constants file updated. (Source: peerless engagement, Venkat B 2026-04-15; FK-034 written 2026-05-22.)
...
```
Read commons/docs/PLANNING_CONTEXT.md and docs/CHUNK_PROGRESS.md, then start next chunk.
```
