...
CHUNK 2 — MuleSoft Design Standards  [ ] REDO
...
CHUNK 3 — Scenario Files             [ ] REDO
  Rewrite all 15 existing scenario files (A-O) with:
    - integrationStyle added to decisions.json defaults
    - compensationStrategy in errorHandling
    - flowControl block with messageTtl, maxConcurrency
    - invalidMessageChannel where applicable
    - EDA fit assessment note where pattern is EDA-based
  Create 3 new scenario files:
...
CHUNK 6 — Story Library              [ ] NOT STARTED
  13+ MuleSoft-specific story template files in story-library/
  Must cover: all primary patterns, global stories, security stories

CHUNK 7 — XML + DWL Templates        [ ] NOT STARTED
  templates/connectors/ (all connector XML stubs)
  scaffold/xml-templates/ (all Mule project template files)
  Must include: wire-tap template, invalid-message-channel template,
                claim-check template, idempotency-check template
...
CHUNK 9 — Create Client Repo Script  [ ] NOT STARTED
  ...
  Uses GitHub API (not gh CLI)
  Creates repo, pushes generated code, sets up Codespace

CHUNK 10 — End-to-End Test           [ ] NOT STARTED
  Full pipeline test using LeoLabs intake docs
  Runs: Analyst → Architect → PM → Scaffold → (manual) repo check
  Validates: decisions.json correct, code compiles, MUnit passes
```
...
```
Read docs/PLANNING_CONTEXT.md and docs/CHUNK_PROGRESS.md, then start Chunk {N}.
```

That is the complete kickoff. No files to paste — the context doc has everything.
