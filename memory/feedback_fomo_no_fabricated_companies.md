...
- Scout Step 0c-3 does real web research → populates `company_context.json competitorFOMO[]` with `sourceUrl`, `sourceLabel`, and `displayName`
- Scout Session 3a copies those entries into `proposal-content.json fomo[]` including `sourceUrl` + `sourceLabel`
...
**Naming rule (enforced in Scout toml):**
