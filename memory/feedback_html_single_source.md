...
description: commons/branding/HTML_DESIGN_STANDARDS.md is the only source for all HTML/UI/flyer styling — never invent new CSS patterns
metadata: 
  node_type: memory
  type: feedback
...
`commons/branding/HTML_DESIGN_STANDARDS.md` is the single source of truth for all UI output in this system — proposals, intake forms, flyers, portals, or any other HTML document.
...
**How to apply:**
- Before writing CSS for any HTML file, read `commons/branding/HTML_DESIGN_STANDARDS.md` first
...
**UI feedback persistence rule:** When the user gives any negative feedback on UI/HTML output, immediately save it to all three places: `HTML_DESIGN_STANDARDS.md` (What NOT to Do or relevant section), a memory file, and `CLAUDE.md`. Do not wait to be asked. No UI mistake should ever repeat across sessions.
