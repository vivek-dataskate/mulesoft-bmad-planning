...
**Why:** Simplify the repo so planning artifacts, scaffold code, and reusable system IP each have a clear home. Old structure mixed doc templates, story templates, and connector stubs in `templates/` alongside `story-library/` and `commons/playbooks/` at different depths.

**New canonical structure:**

| Path | Purpose |
|------|---------|
| `playbooks/{system}/` | System-specific IP — PLAYBOOK.md, DWL transforms, XML sub-flows. Was `commons/playbooks/`. |
...
**How to apply:** Whenever creating new files, use the new paths. All agent TOMLs, generate.js, run-pipeline.js, generate-capabilities.js, and capabilities.yml have been updated. Do NOT use old paths like `commons/playbooks/`, `story-library/`, `templates/connectors/`, or `MULESOFT_DESIGN_STANDARDS.md`.
