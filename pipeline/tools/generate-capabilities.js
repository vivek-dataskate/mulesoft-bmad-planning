...
'use strict';
/**
 * pipeline/tools/generate-capabilities.js
 *
 * Reads all three registries + playbooks + client projects → generates
 * docs/capabilities/index.html (served via GitHub Pages).
 *
 * Usage:
 *   node pipeline/tools/generate-capabilities.js
 *   node pipeline/tools/generate-capabilities.js --out /custom/path/index.html
 *
 * Sources:
 *   mulesoft/connector-registry.json     — Tier 0: connector catalog
 *   mulesoft/snippet-registry.json       — Tier 1/2/3: all capabilities
 *   mulesoft/exchange-registry.json      — Tier 3: Exchange assets (if exists)
 *   mulesoft/playbooks/{system}/{system}_playbook.md         — system playbooks
...
function loadConnectors() {
  const reg = readJSON(path.join(REPO_ROOT, 'mulesoft', 'connector-registry.json'));
  if (!reg) return [];
  const rows = [];
...
function loadSnippets() {
  const reg = readJSON(path.join(REPO_ROOT, 'mulesoft', 'snippet-registry.json'));
...
function loadPlaybooks() {
  const playbooksDir = path.join(REPO_ROOT, 'mulesoft', 'playbooks');
...
function playbookCard(p) {
  ...
  <div class="pb-body">
    ...
    ${objectsHtml}
    ${authHtml}
    <a href="../../mulesoft/playbooks/${esc(p.name)}/${esc(p.name)}_playbook.json" class="pb-link" onclick="event.stopPropagation()">
      View full playbook →
    </a>
  </div>
</div>`;
...
<footer>
  Generated by <a href="../../pipeline/tools/generate-capabilities.js">pipeline/tools/generate-capabilities.js</a> on ${now} ·
