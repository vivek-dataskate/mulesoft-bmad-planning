...
New integration: Salesforce ↔ SAP
  ...
  SAP side:         Build playbooks/sap/ (create once, reused forever)
...
| 2026-05-11 | mrn-healthcare (Scout) | 10-flow engagement: Gravity Forms + Shopify x2 + Mailchimp + SFTP vendors all targeting Salesforce. Dual-target flow (UC1 → SF + Mailchimp). Utilization custom objects required. External ID fields not yet created. State/Country Picklists and multi-currency flags open. | observation |

*Third Salesforce client → evaluate promoting stable patterns to DESIGN_STANDARDS.md.*
