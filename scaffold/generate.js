...
// Generate .claude/ and _bmad/custom/ files in the client repo so the developer
// has the push command and dev agent verification (VR/CS) available from day one.
//
// Files generated:
//   .claude/commands/push.md           — /push slash command (git → PR → merge)
//   .claude/settings.json              — minimal Claude Code settings (no MCP credentials)
//   _bmad/custom/bmad-agent-dev.toml  — dev agent override: VR verify-stories, CS check-story, sprint status update
function genClaudeFiles(d, outDir) {
  const client      = d.project?.client ?? 'client';
  const displayName = client.replace(/[-_]+/g, ' ').replace(/\b\w/g, c => c.toUpperCase());

  // .claude/commands/push.md — full auto-merge push for small fixes (same as planning repo)
  ...
  if (fs.existsSync(pushSrc)) {
    ...
    console.warn('  ⚠  .claude/commands/push.md not found in planning repo — skipping push command');
  }

  // .claude/commands/push-dev.md — PR-only push for story implementation (no auto-merge; Architect reviews)
  ...
  if (fs.existsSync(pushDevSrc)) {
    ...
    console.warn('  ⚠  .claude/commands/push-dev.md not found in planning repo — skipping push-dev command');
  }

  // .claude/settings.json — minimal settings; no MCP credentials (use Codespace secrets)
  const claudeSettings = JSON.stringify({
    "_comment": `Claude Code settings for ${displayName} MuleSoft project. Generated by scaffold.`,
    "mcpServers": {
      "mulesoft": {
        "command": "npx",
  ...
  // _bmad/custom/bmad-agent-dev.toml — copy the dev agent overrides
  ...
  if (fs.existsSync(devAgentSrc)) {
    ...
    console.warn('  ⚠  _bmad/custom/bmad-agent-dev.toml not found in planning repo — skipping dev agent config');
  }
}
...
function main() {
  ...
  const totalFiles = 6 + (profile === 'regulated' ? 1 : 0)  // project-level
                   ...
                   + 4                                        // push.md, push-dev.md, .claude/settings.json, _bmad/custom/bmad-agent-dev.toml
